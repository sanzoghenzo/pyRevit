...
namespace pyRevitLabs.Emojis {
    public static class Emojis {
        private static Regex _emojiRegex = new Regex(@"\:(?<code>[^\s]+?)\:");

        public const string EmojiSpanTemplate = "<span style='font-family: \"Noto_Color_Emoji variant0\"'></span>";
        ...
        public static string Emojize(string input) {
            return _emojiRegex.Matches(input)
                .Cast<Match>()
                .Select(m => m.Groups["code"].Value)
                .Distinct()
                .Where(s => EmojiDict.ContainsKey(s))
                .Accumulate(
                    input, 
                    (input, emoji) => input.Replace(emoji, string.Format(EmojiSpanTemplate, EmojiDict[emoji])));
        }
    }
}
